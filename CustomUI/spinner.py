...
from CustomUI.dot import Dot

Builder.load_file(str(path / "spinner.kv"))
...
class DotSpinner(MDBoxLayout):
    ...
    def get_dot_current_index(self, index: int = 0) -> Dot:
        for dot in self.children:
    ...
    def check_active(self):
        if self.active:
            if not self._animating:
                self.clock = Clock.schedule_interval(self._animate_spinner, 1)
    ...
    def _animate_spinner(self, _) -> None:
        ...
        if not self.get_current_active_dot() or (self._current_active_dot_index == index_max):
            dot = self.get_dot_current_index()
        else:
            self._current_active_dot_index += 1
            dot = self.get_dot_current_index(self._current_active_dot_index)
        dot.active = True
