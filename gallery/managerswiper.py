...
classes MySwiperManager(BoxLayout):
    pass


classes MyCard(MDCard):
    text = StringProperty('')
...
class ItemPagination(ThemableBehavior, Widget):
    ...
    def __init__(self, **kwargs):
        ...
        if not self.color_round_not_active:
            self.color_round_not_active = get_color_from_hex("#0f2f45")
    ...
    def on_touch_move(self, touch):
        if self.collide_point(*touch.pos) and not self.swipe:
            ...
            for widget in Window.children:
                if widget.__class__ is MDNavigationLayout and widget.state == "open":
                    return
            if touch.x < Window.width - 10:
                direction = "left" if self._x > touch.x else "right"
                self.swipe = True
